        ...
        while True:
            ...
            for record in records:
                ...
                if record.error():
                    ...
                    #print('call me======:',msgValue["Temperature"])
                    #將資料寫入ES
                    at = pytz.timezone('Asia/Taipei')
                    dt = datetime.strptime(msgValue["timestamp"], "%Y-%m-%d %H:%M:%S").replace(tzinfo=at)
                    
                    es_id = time.time()

                    #print('es_id:',es_id)msgVal
                    doc = {"device_id": msgValue["device_id"], "timestamp": dt, "logtime": msgValue['timestamp'],
                        "Temperature": msgValue["Temperature"]}
                    #print(doc)
                    res = es.index(index='ltu-iot', doc_type='elk', id=es_id, body=doc)
                    
                    print("%s, rd= %s %s"%(dt, msgValue["timestamp"], res['result']))
